defmodule EventodbEx.LastTest do
  ...
  test "LAST-001: Last message from non-empty stream", %{client: client} do
    ...
    {:ok, last_msg, _client} = EventodbEx.stream_last(client, stream)
  ...
  test "LAST-002: Last message from empty stream", %{client: client} do
    ...
    assert {:ok, nil, _client} = EventodbEx.stream_last(client, stream)
  end
  ...
  test "LAST-003: Last message filtered by type", %{client: client} do
    ...
    {:ok, last_msg, _client} = EventodbEx.stream_last(client, stream, %{type: "TypeB"})
  ...
  test "LAST-004: Last message type filter no match", %{client: client} do
    ...
    assert {:ok, nil, _client} = EventodbEx.stream_last(client, stream, %{type: "TypeB"})
  end
end
