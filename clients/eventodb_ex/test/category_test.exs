defmodule EventodbEx.CategoryTest do
  ...
  test "CATEGORY-001: Read from category", %{client: client} do
    ...
    {:ok, messages, _client} = EventodbEx.category_get(client, category)
  ...
  test "CATEGORY-002: Read category with position filter", %{client: client} do
    ...
    {:ok, messages, _client} =
      EventodbEx.category_get(client, category, %{position: target_gpos})
  ...
  test "CATEGORY-003: Read category with batch size", %{client: client} do
    ...
    {:ok, messages, _client} = EventodbEx.category_get(client, category, %{batch_size: 10})
  ...
  test "CATEGORY-004: Category message format", %{client: client} do
    ...
    {:ok, [msg], _client} = EventodbEx.category_get(client, category)
  ...
  test "CATEGORY-005: Category with consumer group", %{client: client} do
    ...
    {:ok, messages_0, _client} =
      EventodbEx.category_get(client, category, %{
  ...
  test "CATEGORY-006: Category with correlation filter", %{client: client} do
    ...
    {:ok, _result, client} = EventodbEx.stream_write(client, stream1, message1)
    ...
    {:ok, _result, client} = EventodbEx.stream_write(client, stream2, message2)

    # Filter by workflow correlation
    {:ok, messages, _client} =
      EventodbEx.category_get(client, category, %{correlation: "workflow"})
  ...
  test "CATEGORY-007: Read from empty category", %{client: client} do
    ...
    assert {:ok, [], _client} = EventodbEx.category_get(client, category)
  end
