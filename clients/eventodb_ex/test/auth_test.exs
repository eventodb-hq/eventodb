defmodule EventodbEx.AuthTest do
  ...
  test "AUTH-001: Valid token authentication" do
    ...
    assert {:ok, _result, _client} = EventodbEx.stream_write(client, stream, message)
  ...
  test "AUTH-003: Invalid token format" do
    client = EventodbEx.Client.new(base_url(), token: "invalid-token")
    stream = unique_stream()
    message = %{type: "TestEvent", data: %{}}
  ...
  test "AUTH-004: Token namespace isolation" do
    ...
    # Write to ns1
    {:ok, _result, _client1} = EventodbEx.stream_write(client1, stream, message)

    # Try to read from ns2 using same stream name
    {:ok, messages, _client2} = EventodbEx.stream_get(client2, stream)
