defmodule EventodbEx.WriteTest do
  ...
  test "WRITE-002: Write message with metadata", %{client: client} do
    ...
    assert {:ok, result, client} = EventodbEx.stream_write(client, stream, message)
    assert result.position >= 0

    # Read back and verify metadata
    {:ok, [read_msg], _client} = EventodbEx.stream_get(client, stream)
  ...
  test "WRITE-003: Write with custom message ID", %{client: client} do
    ...
    assert {:ok, result, client} =
             EventodbEx.stream_write(client, stream, message, %{id: custom_id})
  ...
  test "WRITE-004: Write with expected version (success)", %{client: client} do
    ...
    assert {:ok, result, _client} =
             EventodbEx.stream_write(client, stream, message, %{expected_version: 1})
  ...
  test "WRITE-005: Write with expected version (conflict)", %{client: client} do
    ...
    assert {:error, error} =
             EventodbEx.stream_write(client, stream, message, %{expected_version: 5})
