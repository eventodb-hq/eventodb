...
client := eventodb.NewClient("http://localhost:8080",
    eventodb.WithHTTPClient(httpClient))
```
...
```go
// Write message
result, err := client.StreamWrite(ctx, streamName, eventodb.Message{
    Type: "EventType",
...
// Write with options
result, err := client.StreamWrite(ctx, streamName, message, &eventodb.WriteOptions{
    ID: eventodb.StrPtr("custom-uuid"),
    ExpectedVersion: eventodb.Int64Ptr(5),
})
...
// Read with options
messages, err := client.StreamGet(ctx, streamName, &eventodb.GetStreamOptions{
    Position: eventodb.Int64Ptr(10),
    BatchSize: eventodb.IntPtr(100),
})
...
last, err := client.StreamLast(ctx, streamName, &eventodb.GetLastOptions{
    Type: eventodb.StrPtr("EventType"),
})
...
messages, err := client.CategoryGet(ctx, "account", &eventodb.GetCategoryOptions{
    ConsumerGroup: &eventodb.ConsumerGroup{
        Member: 0,
        Size: 4,
    },
})
...
messages, err := client.CategoryGet(ctx, "account", &eventodb.GetCategoryOptions{
    Correlation: eventodb.StrPtr("workflow"),
})
```
...
```go
// Create namespace
result, err := client.NamespaceCreate(ctx, "my-namespace", &eventodb.CreateNamespaceOptions{
    Description: eventodb.StrPtr("My test namespace"),
})
...
go func() {
    for {
        ...
        case poke := <-sub.Events:
            ...
            messages, _ := client.StreamGet(ctx, poke.Stream, &eventodb.GetStreamOptions{
                Position: &poke.Position,
                BatchSize: eventodb.IntPtr(1),
            })
...
sub, err := client.SubscribeCategory(ctx, "account", &eventodb.SubscribeCategoryOptions{
    ConsumerGroup: &eventodb.ConsumerGroup{
        Member: 0,
        Size: 4,
    },
...
sub, err := client.SubscribeStream(ctx, "account-123", &eventodb.SubscribeStreamOptions{
    Position: eventodb.Int64Ptr(10),
})
```
...
```go
result, err := client.StreamWrite(ctx, stream, message, &eventodb.WriteOptions{
    ExpectedVersion: eventodb.Int64Ptr(5),
})
if err != nil {
...
```go
// String pointer
description := eventodb.StrPtr("my description")
