...
1. MessageDB server running on `http://localhost:8080`
...
```bash
# Make sure MessageDB server is running
...
MESSAGEDB_URL=http://localhost:9000 mix test
...
```elixir
# In iex
iex> client = MessagedbEx.Client.new("http://localhost:8080")

# Create a namespace
iex> {:ok, ns, client} = MessagedbEx.namespace_create(client, "my-app")

# Write a message
iex> message = %{type: "UserCreated", data: %{email: "user@example.com"}}
iex> {:ok, result, client} = MessagedbEx.stream_write(client, "user-123", message)
iex> result.position
0

# Read it back
iex> {:ok, [msg], client} = MessagedbEx.stream_get(client, "user-123")
...
- `lib/messagedb_ex.ex` - Main public API
- `lib/messagedb_ex/client.ex` - HTTP/RPC client
- `lib/messagedb_ex/types.ex` - Type specifications
- `lib/messagedb_ex/error.ex` - Error handling
...
Make sure the MessageDB server is running in test mode or has auth disabled for namespace creation.
...
{:ok, namespaces, _} = MessagedbEx.namespace_list(client)

# Delete test namespaces
Enum.each(namespaces, fn ns ->
  if String.starts_with?(ns.namespace, "test-") do
    MessagedbEx.namespace_delete(client, ns.namespace)
  end
end)
```
