defmodule MessagedbEx.ReadTest do
  ...
  test "READ-001: Read from empty stream", %{client: client} do
    ...
    assert {:ok, messages, _client} = MessagedbEx.stream_get(client, stream)
    assert messages == []
  end
  ...
  test "READ-003: Read multiple messages", %{client: client} do
    ...
    {:ok, messages, _client} = MessagedbEx.stream_get(client, stream)
  ...
  test "READ-004: Read with position filter", %{client: client} do
    ...
    {:ok, messages, _client} = MessagedbEx.stream_get(client, stream, %{position: 5})
    assert length(messages) == 5
  ...
  test "READ-005: Read with global position filter", %{client: client} do
    ...
    {:ok, messages, _client} =
      MessagedbEx.stream_get(client, stream, %{global_position: target_gpos})
  ...
  test "READ-006: Read with batch size limit", %{client: client} do
    ...
    {:ok, messages, _client} = MessagedbEx.stream_get(client, stream, %{batch_size: 10})
  ...
  test "READ-007: Read with batch size unlimited", %{client: client} do
    ...
    {:ok, messages, _client} = MessagedbEx.stream_get(client, stream, %{batch_size: -1})
