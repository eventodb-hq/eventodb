defmodule MessagedbEx.SseTest do
  ...
  test "SSE-001: Subscribe to stream", %{client: client} do
    ...
    {:ok, _pid} =
      MessagedbEx.subscribe_to_stream(client, stream,
        name: "sse-001",
        position: 0,
    ...
    {:ok, result, _client} = MessagedbEx.stream_write(client, stream, message)
    ...
    # Cleanup
    MessagedbEx.Subscription.close("sse-001")
  end
  ...
  test "SSE-002: Subscribe to category", %{client: client} do
    ...
    # Cleanup
    MessagedbEx.Subscription.close("sse-002")
  end
  ...
  test "SSE-003: Subscribe with position", %{client: client} do
    ...
    # Cleanup
    MessagedbEx.Subscription.close("sse-003")
  end
  ...
  test "SSE-004: Subscribe without authentication" do
    ...
    case result do
      {:ok, _pid} ->
        ...
        MessagedbEx.Subscription.close("sse-004")
  ...
  test "SSE-005: Subscribe with consumer group", %{client: client} do
    ...
    Enum.each(streams, fn stream ->
      MessagedbEx.stream_write(client, stream, message)
    ...
    # Cleanup
    MessagedbEx.Subscription.close("sse-005")
  end
  ...
  test "SSE-006: Multiple subscriptions", %{client: client} do
    ...
    {:ok, _pid1} =
      MessagedbEx.subscribe_to_stream(client, stream1,
        name: "sse-006-a",
        on_poke: fn poke -> send(test_pid, {:poke1, poke}) end
      )

    {:ok, _pid2} =
      MessagedbEx.subscribe_to_stream(client, stream2,
        name: "sse-006-b",
    ...
    # Write to stream1
    message = %{type: "TestEvent", data: %{}}
    {:ok, _result, client} = MessagedbEx.stream_write(client, stream1, message)
    ...
    # Write to stream2
    {:ok, _result, _client} = MessagedbEx.stream_write(client, stream2, message)
    ...
    # Cleanup
    MessagedbEx.Subscription.close("sse-006-a")
    MessagedbEx.Subscription.close("sse-006-b")
  end
  ...
  test "SSE-008: Poke event parsing", %{client: client} do
    ...
    # Cleanup
    MessagedbEx.Subscription.close("sse-008")
  end
  ...
  test "Subscription name must be unique", %{client: client} do
    ...
    # Cleanup
    MessagedbEx.Subscription.close("duplicate-name")
  end
  ...
  test "Close subscription by name", %{client: client} do
    ...
    # Close subscription
    assert :ok = MessagedbEx.Subscription.close("close-test")
