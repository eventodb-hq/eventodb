...
{:ok, _pid} = MessagedbEx.subscribe_to_stream(client, "account-123",
  name: "my-account-sub",
  on_poke: fn poke ->
...
# Close subscription
MessagedbEx.Subscription.close("my-account-sub")
```
...
```elixir
MessagedbEx.subscribe_to_stream(client, stream_name, opts)
```
...
```elixir
MessagedbEx.subscribe_to_category(client, category_name, opts)
```
...
```elixir
MessagedbEx.Subscription.close(name)
```
...
{:ok, _pid} = MessagedbEx.subscribe_to_stream(client, "account-123",
  ...
  on_poke: fn poke ->
    ...
    {:ok, messages, _} = MessagedbEx.stream_get(client, poke.stream,
...
{:ok, _pid} = MessagedbEx.subscribe_to_category(client, "account",
  name: "worker-1",
  consumer_group: %{member: 0, size: 4},
  on_poke: fn poke ->
    {:ok, messages, _} = MessagedbEx.category_get(client, "account",
...
defmodule MyProcessor do
  ...
  def init({client, category}) do
    ...
    {:ok, _pid} = MessagedbEx.subscribe_to_category(client, category,
  ...
  def terminate(_reason, state) do
    MessagedbEx.Subscription.close("#{state.category}-processor")
  end
end
```
...
{:ok, _} = MessagedbEx.subscribe_to_category(client, "account",
  name: "processor",
